...
/**
  ...
  * @date : 2019/10/17  
  **/
class PyXGBoostClassificationModel(val model:XGBoostClassificationModel) {
  ...
  def transform(predictData : org.apache.spark.sql.Dataset[_],features : java.util.List[String]):DataFrame ={
    ...
    val xgbInputDense=xgbInput.map(row=>{PredictData(row.getAs[org.apache.spark.ml.linalg.Vector]("tempFeatures").toDense)}).toDF("features")
